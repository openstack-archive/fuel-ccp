...
# grep automatically stripes out non-printable control characters from kubectl output:
K8S_API_SERVER_URL=`kubectl cluster-info | grep --color=never "Kubernetes master" | grep -o "http.*"`
sed -i "s#K8S_API_SERVER_URL_CHANGE_ME#${K8S_API_SERVER_URL}#" ${POD_YAML_FILE}
