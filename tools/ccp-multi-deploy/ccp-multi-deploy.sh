        ...
        if [ ${cnt} -eq 300 ]; then
            ...
            if [ -n "${PRINT_STD}" ]; then
                ...
                for f in `${CCP} status | awk '/wip/{print $2}'`; do
                    set +e
                    for p in `kubectl -n "${1}" get pods | grep "${f}" | awk '{print $1}'`; do
                        kubectl -n "${NAMESPACE}" describe pod "${p}"
                        if kubectl -n ${1} get pods "${p}" -o jsonpath={.spec.containers[*].name}; then
                            CONTAINERS_NAMES=`kubectl -n ${1} get pods "${p}" -o jsonpath={.spec.containers[*].name}`
