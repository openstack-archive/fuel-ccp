...
-v, --openstack-version=VERSION
        set openstack version newton or master
EOF
    exit
}
...
OPTS=`getopt -o 'hsnv:' --long help,skip-building-images,number-of-envs: -n 'parse-options' -- ${@}`
if [ ${?} != 0 ] ; then
...
while true; do
    case ${1} in
        ...
        -v|--openstack-version ) VERSION=${2}; shift; shift ;;
        -- ) shift; break ;;
        * ) break ;;
    esac
done
...
# Fetch CCP repos
CCP="ccp --debug --config-file ${CONFIG_DIR}/ccp-cli-${VERSION}-config-1.yaml"
${CCP} fetch
...
for n in $(seq 1 ${NUMBER_OF_ENVS}); do
    CCP="ccp --debug --config-file ${CONFIG_DIR}/ccp-cli-${VERSION}-config-${n}.yaml"
    ${CCP} deploy
