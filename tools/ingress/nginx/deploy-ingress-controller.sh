...
sleep 1
sed -e "s/\bHTTP_PORT\b/${HTTP_PORT}/g" -e "s/\bNAMESPACE\b/${NAMESPACE}/g" ${workdir}/default-backend.yaml | kube_cmd apply -f -
sed -e "s/\bHTTP_PORT\b/${HTTP_PORT}/g" -e "s/\bHTTPS_PORT\b/${HTTPS_PORT}/g" -e "s/\bNAMESPACE\b/${NAMESPACE}/g" \
    -e "s/\bEXTERNAL_IP\b/${EXTERNAL_IP}/g" ${workdir}/controller.yaml | kube_cmd create -f -
if [ -n "${CLEANUP}" ]; then
    rm $TLS_KEY $TLS_CERT
fi
