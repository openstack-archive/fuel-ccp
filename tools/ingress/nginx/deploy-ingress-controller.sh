...
HTTP_PORT=80
HTTPS_PORT=8443
SERVICE_NAME="default-web-backend"
SERVICE_PORT=8080
...
kube_cmd create secret tls nginx-ingress-controller-cert --cert=$TLS_CERT --key=$TLS_KEY
kube_cmd create -f ${workdir}/nginx-config.yaml
kube_cmd create configmap nginx-template --from-file=nginx.tmpl=./nginx.tmpl
sleep 1
sed -e "s/\bHTTP_PORT\b/${HTTP_PORT}/g" -e "s/\bHTTPS_PORT\b/${HTTPS_PORT}/g" -e "s/\bNAMESPACE\b/${NAMESPACE}/g" \
    -e "s/\bEXTERNAL_IP\b/${EXTERNAL_IP}/g" -e "s/SERVICE_NAME\b/${SERVICE_NAME}/g" \
    -e "s/SERVICE_PORT\b/${SERVICE_PORT}/g" -e "s/DOMAIN/${DOMAIN}/g" ${workdir}/controller.yaml | kube_cmd create -f -
if [ -n "${CLEANUP}" ]; then
    rm $TLS_KEY $TLS_CERT
fi
