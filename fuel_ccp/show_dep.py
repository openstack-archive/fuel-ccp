...
class Node(object):
    ...
    def __init__(self, name, sort, dependencies=[], job_parent=None):
        self.name = name
        self.sort = sort
...
def show_dep(components):
    ...
    deps = set()
    for component in components:
        deps.update(_get_deps(component, deps_map))
    print(" ".join(deps))
