...
def _wait_until_empty(command, attempts, *args, **kwargs):
    while attempts > 0:
...
def _cleanup_servers(session):
    ...
    server_list = _wait_until_empty(
        nova.servers.list, 60, search_opts={"all_tenants": True})
    if server_list:
...
def _cleanup_network_resources(session):
    ...
    subnet_list = _wait_until_empty(neutron.list_subnets, 10)
    if subnet_list:
        raise RuntimeError(
    ...
    network_list = _wait_until_empty(neutron.list_networks, 10)
    if network_list:
        raise RuntimeError(
