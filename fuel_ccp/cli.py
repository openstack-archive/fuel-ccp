...
class ActionStatus(BaseCommand, lister.Lister):
    ...
    def take_action(self, parsed_args):
        ...
        elif parsed_args.action > 1:
            return get_statuses_for_actions(
                action.get_action_statuses_by_names(parsed_args.action))
        else:
            return get_status_for_single_action(
                action.get_action_status_by_name(parsed_args.action))
...
def get_statuses_for_actions(action_obects):
    return (
        ACTION_FIELDS,
        ((a.name, a.component, a.date, a.status, a.restarts)
         for a in action_obects)
    )
