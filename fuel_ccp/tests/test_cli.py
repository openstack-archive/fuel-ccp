...
class TestDeploy(TestParser):
    ...
    def test_parser(self):
        ...
        self._run_app()
        dc_mock.assert_called_once_with(components=self.components)
        for k, v in self.action_vals.items():
            self.assertEqual(self.conf.action[k], v)
...
class TestFetch(TestParser):
    ...
    def test_parser(self):
        self._run_app()
        self.fetch_mock.assert_called_once_with(self.conf.repositories.names)


class TestCleanup(TestParser):
    cmd = 'cleanup'
    scenarios = [
        ('empty', {'argv': [], 'margs': {'auth_url': None, 'skip_os_cleanup': False}}),
        ('auth_url', {
