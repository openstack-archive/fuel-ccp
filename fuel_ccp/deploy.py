...
def create_dependencies(workflows, components_map):
    for name, wf in six.iteritems(workflows):
        # FIXME Assuming that we have only 2 types for now - job and service
        child = ("service/%s" % name if name in components_map
                 else "job/%s" % name)
        for dep in wf['workflow']['dependencies']:
            parent = ("service/%s" % dep if dep in components_map
                      else "job/%s" % dep)
