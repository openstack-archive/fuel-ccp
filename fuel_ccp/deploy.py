...
def _make_topology(nodes, roles, replicas):
    ...
    for svc, svc_hosts in six.iteritems(service_to_node):
        ...
        if svc_replicas < 1:
            LOG.error("Replicas should be positive numbers while requested %s "
                      "replicas for service %s", svc_replicas, svc)
            raise RuntimeError("Replicas should be positive numbers")
