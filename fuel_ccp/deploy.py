...
def _process_ports(service):
    ...
    for port in service["ports"]:
        ...
        if CONF.configs.use_ingress and port.get("ingress_name"):
            ingress_host = '.'.join((
                port["ingress_name"], CONF.kubernetes.namespace,
                CONF.configs.ingress_domain))
            if ingress_host:
...
def _create_openrc(config):
    openrc = [
        ...
        '.'.join((CONF.configs.keystone.ingress_name,
