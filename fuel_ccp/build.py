...
def create_rendered_dockerfile(path, name, tmp_path, config):
    def copy_sources(project_name, cont_dir):
        ...
        if not git_url and not source_dir:
            LOG.error('%s: Neither git_url nor source_dir specified for project'
                      '%s (exactly one should be specified)',
                      name, project_name)
            sys.exit(1)

        if git_url and source_dir:
            LOG.error('%s: Both git_url and source_dir specified for project'
                      '%s (exactly one should be specified)',
                      name, project_name)
            sys.exit(1)
